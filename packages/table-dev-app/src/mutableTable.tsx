...
export class MutableTable extends React.Component<{}, IMutableTableState> {
  ...
  public render() {
    ...
    return (
      <div className="container">
        ...
        >
          <div className={layoutBoundary ? "layout-boundary" : "layout-passthrough-fill"}>{this.renderTable()}</div>
        </SlowLayoutStack>
        {this.renderSidebar()}
      </div>
    );
  ...
  private renderCell = (rowIndex: number, columnIndex: number) => {
    ...
    if (this.state.enableCellEditing) {
      ...
      return (
        <Cell className={classes} columnIndex={columnIndex} rowIndex={rowIndex} wrapText={this.state.enableCellWrap}>
          {valueAsString}
        </Cell>
      );
    }
