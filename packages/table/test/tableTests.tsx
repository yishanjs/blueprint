...
describe("<Table>", function(this) {
  ...
  describe("Full-table selection", () => {
    ...
    it("Aligns properly with the table borders", () => {
      ...
      const [actualWidthAsNumber, actualHeightAsNumber] = [actualWidth, actualHeight].map(n => parseInt(n, BASE_10));
  ...
  describe("Freezing", () => {
    ...
    it("renders correct number of frozen cells if numFrozenRows = 1 and numFrozenColumns = 1", () => {
      const table = mount(createTableOfSize(NUM_COLUMNS, NUM_ROWS, {}, { numFrozenRows: 1, numFrozenColumns: 1 }));
      ...
      expect(table.find(`.${Classes.TABLE_QUADRANT_TOP_LEFT} .${Classes.TABLE_CELL}`).length).to.equal(NUM_TOP_LEFT);
    });
    ...
    describe("on mount", () => {
      describe("errors", () => {
        ...
        it("throws an error if numFrozenColumns < 0", () => {
          expectPropValidationError(Table, { numFrozenColumns: -1 }, Errors.TABLE_NUM_FROZEN_COLUMNS_NEGATIVE);
        });
