...
describe("TableQuadrantStack", () => {
  ...
  describe("Initial render", () => {
    ...
    it("invokes rowHeaderCellRenderer once for each quadrant on mount", () => {
      ...
      mount(
        <TableQuadrantStack grid={grid} bodyRenderer={bodyRenderer} rowHeaderCellRenderer={rowHeaderCellRenderer} />,
      );
    ...
    it("does not render LEFT/TOP_LEFT quadrants if row header not shown and no frozen columns", () => {
      const component = mount(<TableQuadrantStack grid={grid} bodyRenderer={sinon.spy()} enableRowHeader={false} />);
  ...
  describe("Resize callbacks", () => {
    ...
    describe("on row resize", () => {
      it("doesn't throw an error if handleRowResizeGuide not provided", () => {
        mount(
          <TableQuadrantStack grid={grid} bodyRenderer={sinon.spy()} rowHeaderCellRenderer={renderRowOrColumnHeader} />,
        );
        expect(() => resizeHandler([])).not.to.throw();
      });
  ...
  describe("Size syncing", () => {
    ...
    function assertQuadrantSizesCorrectIfRowHeadersHidden(numFrozenRows: number, numFrozenColumns: number) {
      ...
      const expectedHeight =
        COLUMN_HEADER_HEIGHT + (numFrozenRows === 0 ? EXPECTED_HEADER_BORDER_WIDTH : numFrozenRows * ROW_HEIGHT);
    ...
    function assertNonMainQuadrantSizesCorrect(component: HTMLElement, expectedWidth: number, expectedHeight: number) {
  ...
  describe("Scroll syncing", () => {
    ...
    function findQuadrantScrollContainer(element: HTMLElement, quadrantType: QuadrantType) {
      ...
      return element.querySelector(`.${quadrantClass} .${Classes.TABLE_QUADRANT_SCROLL_CONTAINER}`) as HTMLElement;
    }
