...
export class Table extends AbstractComponent2<ITableProps, ITableState, ITableSnapshot> {
  ...
  public static getDerivedStateFromProps(props: ITableProps, state: ITableState) {
    ...
    if (columnWidths !== state.columnWidths || didChildrenChange) {
      ...
      const previousColumnWidths = newChildrenArray.map((child: React.ReactElement<IColumnProps>, index: number) => {
  ...
  private static resizeRowsByApproximateHeightDefaults: Record<keyof IResizeRowsByApproximateHeightOptions, number> = {
  ...
  private maybeRenderFocusHotkeys() {
    ...
    if (enableFocusedCell != null) {
      return [
        ...
        />,
        <Hotkey key="move up" label="Move focus cell up" group="Table" combo="up" onKeyDown={this.handleFocusMoveUp} />,
        <Hotkey
          key="move down"
    ...
    if (nextScrollLeft !== undefined || nextScrollTop !== undefined) {
      ...
      if (nextScrollLeft !== undefined) {
        const leftCorrection =
          this.shouldDisableHorizontalScroll() || this.rowHeaderElement == null ? 0 : this.rowHeaderElement.clientWidth;
