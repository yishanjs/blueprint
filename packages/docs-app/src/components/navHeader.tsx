...
export class NavHeader extends React.PureComponent<INavHeaderProps, {}> {
  ...
  public renderHotkeys() {
    return (
      <Hotkeys>
        <Hotkey global={true} combo="shift + d" label="Toggle dark theme" onKeyDown={this.handleDarkSwitchChange} />
      </Hotkeys>
    );
  }
  ...
  private renderVersionsMenu() {
    ...
    const [current] = /\/versions\/([0-9]+)/.exec(location.href) || [this.props.useNextVersion ? nextVersion : version];
