...
export class PopoverExample extends React.PureComponent<IExampleProps, IPopoverExampleState> {
  ...
  private renderOptions() {
    ...
    return (
      <>
        <H5>Appearance</H5>
        <FormGroup helperText="May be overridden to prevent overflow" label="Position when opened" labelFor="position">
          <HTMLSelect value={this.state.position} onChange={this.handlePositionChange} options={VALID_POSITIONS} />
        </FormGroup>
        <Label>
          Example content
        ...
        />
        <Switch checked={this.state.canEscapeKeyClose} label="Can escape key close" onChange={this.toggleEscapeKey} />
