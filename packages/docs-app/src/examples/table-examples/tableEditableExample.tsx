...
import { Intent } from "@yishanzhilubp/core";
import { Example, IExampleProps } from "@yishanzhilubp/docs-theme";
import { Column, ColumnHeaderCell, EditableCell, EditableName, Table } from "@yishanzhilubp/table";
...
export class TableEditableExample extends React.PureComponent<IExampleProps, ITableEditableExampleState> {
  ...
  public render() {
    const columns = this.state.columnNames.map((_: string, index: number) => {
      return <Column key={index} cellRenderer={this.renderCell} columnHeaderCellRenderer={this.renderColumnHeader} />;
    });
    return (
