...
import { Button, Classes, Menu, MenuItem } from "@yishanzhilubp/core";
...
describe("<DateRangePicker>", () => {
  ...
  describe("reconciliates dayPickerProps", () => {
    ...
    it("disables out-of-range min dates", () => {
      const { left } = wrap(
        <DateRangePicker initialMonth={new Date(2017, Months.AUGUST, 1)} minDate={new Date(2017, Months.AUGUST, 20)} />,
      );
    ...
    describe("event handlers", () => {
      ...
      it("calls onMonthChange on button prev click", () => {
        const onMonthChange = sinon.spy();
        wrap(<DateRangePicker defaultValue={defaultValue} dayPickerProps={{ onMonthChange }} />).clickNavButton("prev");
        assert.isTrue(onMonthChange.called);
      });
  ...
  describe("initially displayed month", () => {
    ...
    it("is today if only maxDate/minDate set and today is in date range", () => {
      const maxDate = new Date(2200, Months.JANUARY);
      const minDate = new Date(2000, Months.JANUARY);
    ...
    it("is a day between minDate and maxDate if only maxDate/minDate set and today is not in range", () => {
      ...
      assert.isTrue(DateUtils.isDayInRange(new Date(leftView.getYear(), leftView.getMonth()), [minDate, maxDate]));
    });
  ...
  describe("minDate/maxDate bounds", () => {
    ...
    it("an error is thrown if value is outside bounds", () => {
      ...
      expectPropValidationError(DateRangePicker, { value, minDate, maxDate }, Errors.DATERANGEPICKER_VALUE_INVALID);
    });
    ...
    const harness = {
      ...
      assertSelectedDays: (from?: number, to?: number) => {
        ...
        if (from != null && to != null) {
          assert.lengthOf(harness.getDays(DateClasses.DATERANGEPICKER_DAY_SELECTED_RANGE), Math.max(0, to - from - 1));
        }
      },
  ...
  function wrapDayPicker(parent: ReactWrapper<IDateRangePickerProps, IDateRangePickerState>, which: "left" | "right") {
    const harness = {
      get wrapper() {
