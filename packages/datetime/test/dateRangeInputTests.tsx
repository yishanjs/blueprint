...
describe("<DateRangeInput>", () => {
  ...
  it("passes custom classNames to popover wrapper", () => {
    ...
    const wrapper = mount(
      <DateRangeInput {...DATE_FORMAT} className={CLASS_1} popoverProps={{ className: CLASS_2, usePortal: false }} />,
    );
  ...
  describe("when uncontrolled", () => {
    ...
    it("Clicking a date invokes onChange with the new date range and updates the input fields", () => {
      ...
      const { root, getDayElement } = wrap(
        <DateRangeInput {...DATE_FORMAT} closeOnSelection={false} defaultValue={defaultValue} onChange={onChange} />,
      );
      root.setState({ isOpen: true });
      root.update();
    ...
    it("Clearing the dates in both inputs invokes onChange with [null, null] and leaves the inputs empty", () => {
      const onChange = sinon.spy();
      const { root } = wrap(<DateRangeInput {...DATE_FORMAT} onChange={onChange} defaultValue={[START_DATE, null]} />);
      getStartInput(root).simulate("focus");
      changeStartInputText(root, "");
      expect(onChange.called).to.be.true;
  ...
  describe("when controlled", () => {
    ...
    it("Pressing Enter saves the inputted date and closes the popover", () => {
      const onChange = sinon.spy();
      const { root } = wrap(<DateRangeInput {...DATE_FORMAT} onChange={onChange} value={[undefined, undefined]} />);
      root.setState({ isOpen: true });
    ...
    it("Clicking a date invokes onChange with the new date range and updates the input field text", () => {
      const onChange = sinon.spy();
      const { root, getDayElement } = wrap(<DateRangeInput {...DATE_FORMAT} value={DATE_RANGE} onChange={onChange} />);
    ...
    describe("Typing an invalid date", () => {
      ...
      beforeEach(() => {
        ...
        const result = wrap(
          <DateRangeInput {...DATE_FORMAT} invalidDateMessage={INVALID_MESSAGE} onError={onError} value={DATE_RANGE} />,
        );
        root = result.root;
    ...
    it(`Clearing only the start input (e.g.) invokes onChange with [null, <endDate>], doesn't clear the\
      ...
      const onChange = sinon.spy();
      const { root, getDayElement } = wrap(<DateRangeInput {...DATE_FORMAT} onChange={onChange} value={DATE_RANGE} />);
