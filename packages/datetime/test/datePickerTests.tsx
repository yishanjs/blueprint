...
describe("<DatePicker>", () => {
  ...
  describe("reconciliates dayPickerProps", () => {
    ...
    it("doesn't show outside days if enableOutsideDays=false", () => {
      ...
      const wrapper = mount(<DatePicker defaultValue={defaultValue} dayPickerProps={{ showOutsideDays: false }} />);
  ...
  describe("initially displayed month", () => {
    ...
    it("is a day between minDate and maxDate if today is not in range", () => {
      ...
      assert.isTrue(
        DateUtils.isDayInRange(new Date(root.state("displayYear"), root.state("displayMonth")), [minDate, maxDate]),
      );
    });
  ...
  describe("minDate/maxDate bounds", () => {
    ...
    it("an error is not thrown if initialMonth is outside day bounds but inside month bounds", () => {
      assert.doesNotThrow(() =>
        wrap(<DatePicker initialMonth={new Date(2015, Months.JANUARY, 12)} minDate={MIN_DATE} maxDate={MAX_DATE} />),
      );
    });
  ...
  describe("time selection", () => {
    ...
    it("onChange fired when the time is changed", () => {
      const onChangeSpy = sinon.spy();
      const { root } = wrap(
        <DatePicker defaultValue={defaultValue} onChange={onChangeSpy} timePickerProps={{ showArrowButtons: true }} />,
      );
