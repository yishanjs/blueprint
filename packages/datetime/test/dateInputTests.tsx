...
import { Classes as CoreClasses, InputGroup, Intent, Keys, Popover, Position } from "@yishanzhilubp/core";
...
describe("<DateInput>", () => {
  ...
  it("supports custom input props", () => {
    const wrapper = mount(<DateInput {...DATE_FORMAT} inputProps={{ style: { background: "yellow" }, tabIndex: 4 }} />);
  ...
  describe("when uncontrolled", () => {
    ...
    it("Popover doesn't close when month changes", () => {
      ...
      const { root, changeSelect } = wrap(
        <DateInput {...DATE_FORMAT} defaultValue={defaultValue} popoverProps={{ isOpen: true, usePortal: false }} />,
      );
      changeSelect(Classes.DATEPICKER_MONTH_SELECT, Months.FEBRUARY);
      assert.isTrue(root.find(Popover).prop("isOpen"));
    });
  ...
  describe("when controlled", () => {
    ...
    it("Pressing Enter saves the inputted date and closes the popover", () => {
      const onKeyDown = sinon.spy();
      const onChange = sinon.spy();
      const { root } = wrap(<DateInput {...DATE_FORMAT} inputProps={{ onKeyDown }} onChange={onChange} value={DATE} />);
      root.setState({ isOpen: true });
    ...
    it("Typing in a date invokes onChange and inputProps.onChange", () => {
      ...
      const onInputChange = sinon.spy();
      const wrapper = mount(
        <DateInput {...DATE_FORMAT} inputProps={{ onChange: onInputChange }} onChange={onChange} value={DATE} />,
      );
    ...
    it("Clearing the date in the input invokes onChange with null", () => {
      const onChange = sinon.spy();
      const { root } = wrap(<DateInput {...DATE_FORMAT} value={new Date(2016, Months.JULY, 22)} onChange={onChange} />);
      root.find("input").simulate("change", { target: { value: "" } });
      assert.isTrue(onChange.calledWith(null, true));
    });
