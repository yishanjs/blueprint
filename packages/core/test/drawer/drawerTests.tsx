...
describe("<Drawer>", () => {
  ...
  describe("position", () => {
    it("casts angle positions into pure positions (with console warning)", () => {
      ...
      assert.isTrue(drawerTop.find(`.${Classes.DRAWER}`).equals(drawerTopLeft.find(`.${Classes.DRAWER}`).getElement()));
      assert.isFalse(
    ...
    return [
      <div className={Classes.DRAWER_BODY} key={1}>
        <p>
          ...
          magna alqua. Ut enim ad minimum veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea
          commodo consequat.
        </p>
      </div>,
