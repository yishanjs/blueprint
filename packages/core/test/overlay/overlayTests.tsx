...
import { dispatchMouseEvent } from "@yishanzhilubp/test-commons";
...
describe("<Overlay>", () => {
  ...
  describe("onClose", () => {
    ...
    it("not invoked on document mousedown when hasBackdrop=false and canOutsideClickClose=false", () => {
      const onClose = spy();
      mountWrapper(
        <Overlay canOutsideClickClose={false} hasBackdrop={false} isOpen={true} onClose={onClose} usePortal={false}>
          {createOverlayContents()}
        </Overlay>,
      );
