...
describe("Suggest", () => {
  ...
  selectComponentSuite<ISuggestProps<IFilm>, ISuggestState<IFilm>>(props =>
    mount(
      <Suggest {...props} inputValueRenderer={inputValueRenderer} popoverProps={{ isOpen: true, usePortal: false }} />,
    ),
  );
  ...
  describe("Uncontrolled Mode with default value", () => {
    it("initialize the selectedItem with the defaultSelectedItem", () => {
      ...
      assert.strictEqual(wrapper.state().selectedItem, defaultSelectedItem, "The selected item should be initialized");
    });
